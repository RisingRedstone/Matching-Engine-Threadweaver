...
#include "../src/buffer/ring/simple_approach.hpp"
#include "../src/buffer/scsp.hpp"
...
static const int num_of_writers = 1; // this cannot change for this test
...
template <typename Layout>
using ProducerFactory = engine::buffer::scsp::Factory::ProducerFactory<
    engine::buffer::ring::simple_approach::Producer, Layout>;
template <typename Layout>
using ConsumerFactory = engine::buffer::scsp::Factory::ConsumerFactory<
    engine::buffer::ring::simple_approach::Consumer, Layout>;
...
// Had to force an inline here. idk why should be pretty simple really
__attribute__((always_inline)) inline void thread_yield_waiter(int attempt) {
  // if (attempt < 10) {
...
#else
  ull *show_module = new ull[buffer_data_type::max_elems](0);
#endif
  int i = 0;
  int attempts = 0;
